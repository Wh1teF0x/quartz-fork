...
function toggleFolder(evt: MouseEvent) {
  ...
  const currentFolderState = currentExplorerState.find((item) => item.path === folderContainer.dataset.folderpath);
...
function createFolderNode(currentSlug: FullSlug, node: FileTrieNode, opts: ParsedOptions): HTMLLIElement {
  ...
  const isCollapsed =
    currentExplorerState.find((item) => item.path === folderPath)?.collapsed ?? opts.folderDefaultState === 'collapsed';
  ...
  for (const child of node.children) {
    const childNode = child.isFolder ? createFolderNode(currentSlug, child, opts) : createFileNode(currentSlug, child);
...
async function setupExplorer(currentSlug: FullSlug) {
  ...
  for (const explorer of allExplorers) {
    ...
    for (const child of trie.children) {
      const node = child.isFolder ? createFolderNode(currentSlug, child, opts) : createFileNode(currentSlug, child);
