...
import { Root, Html, BlockContent, PhrasingContent, DefinitionContent, Paragraph, Code } from 'mdast';
        ...
        plugins.push(() => {
          return (tree: HtmlRoot, _file) => {
            visit(tree, 'element', (node: Element, _idx, parent) => {
              if (node.tagName === 'code' && ((node.properties?.className ?? []) as string[])?.includes('mermaid')) {
                parent!.children = [
                  {
                    type: 'element',
                    tagName: 'button',
...
declare module 'vfile' {
  interface DataMap {
    ...
    hasLeafletMap: boolean | undefined;
  }
}
